...
```{r}
inv_shifted <- SamsaRaLight::create_square_inventory(data_trees, cell_size, core_polygon = core_polygon)
```
...
```{r out_samsa}
inv_filled <- SamsaRaLight::fill_square_inventory(inv_shifted$new_inventory, 
                                                  inv_shifted$info$cell_size, 
                                                  inv_shifted$info$n_cells_x, 
                                                  inv_shifted$info$n_cells_y,
                                                  core_polygon = inv_shifted$core_polygon)
...
out_filled <- SamsaRaLight::sl_run(
    ...
    cell_size = inv_filled$info$cell_size, 
    n_cells_x = inv_filled$info$n_cells_x, 
    n_cells_y = inv_filled$info$n_cells_y,
    turbid_medium = TRUE,
...
ggplot() +
  ...
  scale_x_continuous(breaks = seq(0, inv_filled$info$n_cells_x * inv_filled$info$cell_size,
                                  by = inv_filled$info$cell_size),
                     labels = round(seq(0, inv_filled$info$n_cells_x * inv_filled$info$cell_size,
                                        by = inv_filled$info$cell_size),
                                    digits = 1)) +
  scale_y_continuous(breaks = seq(0, inv_filled$info$n_cells_y * inv_filled$info$cell_size,
                                  by = inv_filled$info$cell_size),
                     labels = round(seq(0, inv_filled$info$n_cells_y * inv_filled$info$cell_size,
                                        by = inv_filled$info$cell_size),
                                    digits = 1)) +
...
data_sensors <- data_sensors_irres1 %>% 
  dplyr::mutate(x = x + inv_shifted$info$shift_x,
                y = y + inv_shifted$info$shift_y)
