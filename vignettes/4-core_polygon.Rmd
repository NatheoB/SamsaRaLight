---
title: "Tutorial 4 - Consider the inventory zone"
subtitle: "Create an axis-aligned rectangle virtual stand from a polygonial inventory of trees"
...
```{r, fig.dim = c(8, 6)}
core_polygon_df <- data.frame(x = c(0, 171.5, 200, 28.5), y = c(70.5, 139.8, 69.3, 0))

inv_filled_simplePolygon <- SamsaRaLight::fill_rect_stand(data_trees, 
                                                          cell_size, n_cells_x, n_cells_y,
                                                          core_polygon_df)

plot_inv_simplePolygon  <- plot_inv +
  geom_sf(data = inv_filled_simplePolygon$core_polygon_df, 
          inherit.aes = FALSE,
...
```{r}
inv_filled <- SamsaRaLight::fill_rect_stand(data_trees, cell_size, n_cells_x, n_cells_y)
```

```{r output_polygon, fig.dim = c(8, 6)}
plot_inv_polygon <- plot_inv +
  geom_sf(data = inv_filled$core_polygon_df, inherit.aes = FALSE,
          fill = "yellow", color = "black", alpha = 0.7)
