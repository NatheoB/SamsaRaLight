...
```{r, fig.dim = c(8, 6)}
df_core_polygon <- data.frame(x = c(0, 171.5, 200, 28.5), y = c(70.5, 139.8, 69.3, 0))

inv_filled_simplePolygon <- SamsaRaLight::fill_inventory(data_trees, 
...
sprintf("Bechefa area is %f ha with a total basal area of %f m2/ha",
        inv_filled$info_invs$core_area_ha, inv_filled$info_invs$core_batot_m2ha)

sprintf("%i trees added to the initial inventory, for a total basal area of %f m2/ha",
        inv_filled$info_invs$n_added_tree, inv_filled$info_invs$new_batot_m2ha)
```
