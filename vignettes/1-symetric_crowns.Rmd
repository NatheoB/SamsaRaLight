...
## Set the virtual stand geometry

SamsaRaLight runs on a rectangle stand You first need to precise the geometry of the stand with:
...
```{r}
data_sl$output$trees %>% 
  dplyr::mutate(punobs = eunobs / e) %>% 
  dplyr::left_join(data_trees, by = "id_tree") %>% 
  
  ggplot(aes(y = eunobs, x = e, color = species)) +
  geom_point() +
  theme(legend.position = "top") +
  scale_color_manual(values = c("skyblue", "salmon", "olivedrab3"))
...
data_sl$output$trees %>% 
  ...
  ggplot(aes(y = punobs, x = dbh_cm, color = species)) +
  geom_point() +
  theme(legend.position = "top") +
  scale_color_manual(values = c("skyblue", "salmon", "olivedrab3"))
```
...
plot_sl_output(data_sl, plot.trees = FALSE)
```
