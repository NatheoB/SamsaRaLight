...
#' \itemize{
#'   \item Ensures `tree_inv` is a non-empty data.frame with all required columns.
#'   \item Checks that `id_tree` values are unique.
#'   \item Validates numeric columns (`x`, `y`, `dbh_cm`, `h_m`, `hbase_m`, `rn_m`, `rs_m`, `re_m`, `rw_m`) are numeric and non-negative.
#'   \item Verifies that `crown_type` values are one of \code{"E"}, \code{"P"}, \code{"2E"}, \code{"8E"}, or \code{"4P"}.
...
#'   \item Checks that `hmax_m` is provided when required by crown type (\code{"2E"} or \code{"8E"}) and lies between `hbase_m` and `h_m`.
#'   \item Ensures `hbase_m` < `h_m`.
#'   \item Verifies that each tree has at least one crown interception property defined (`crown_openness` or `crown_lad`).
...
check_inventory <- function(tree_inv, verbose = TRUE) {
  ...
  if (any(tree_inv[numeric_cols] < 0, na.rm = TRUE)) stop(
    "Numeric geometric variables must be non-negative.", call. = FALSE
  )
  ...
  ## ---- success --------------------------------------------------------------
  if (verbose) message("Inventory successfully validated: all required inputs are consistent.")
  invisible(TRUE)
}
