...
In the previous vignettes, we examined how virtual sensors can be
implemented in the stand to estimate light arriving on the ground.
Virtual sensors can be used to calibrate crown parameters by minimizing
the discrepancy between observed and simulated light.

In this vignette, we illustrate a simple approach to calibrate the tree
leaf area density (LAD), based on a grid search over LAD values.

## Initialise the stand

Here, we will estimate a mean-site LAD on the Cloture dataset, the one
...
![](5b-virtual_sensors_lad_files/figure-html/unnamed-chunk-6-1.png)

In this example, the optimal LAD is 0.918, confirming that we
...
![](5b-virtual_sensors_lad_files/figure-html/unnamed-chunk-9-1.png)
